...
def save_file(data) -> None:
    with open(
        os.path.expanduser("~/data/thw.json"), mode="w", encoding="utf-8"
    ) as file:
...
def main() -> None:
    ...
    if response.status_code == 200:
        ...
        pattern_cpu = re.compile(
            r"((?:Core (?:Ultra)?|Ryzen)\s*i?[3579](?:\s*|-)\d{3,}[a-zA-Z]+)"
        )
        ...
        for node in nodes:
            ...
            temp["link"] = node.css_first(
                "div.hawk-deal-widget-image-container a"
            ).attrs["href"]
